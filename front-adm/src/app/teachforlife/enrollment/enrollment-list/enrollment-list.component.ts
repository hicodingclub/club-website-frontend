...
export class EnrollmentListComponent extends EnrollmentListCustComponent implements OnInit {
  ...
  constructor(public enrollmentService: EnrollmentService, public injector: Injector, public router: Router, public route: ActivatedRoute, public location: Location) {
    ...
    this.stringFields = ['name', 'email', 'status', ];
    this.referenceFields = ['tutor', ];
    this.dateFields = ['createdAt', ];
    this.numberFields = ['grade', ];
    this.stringBoxFields = ['name', 'email', 'status', ];
    this.ownSearchFields = ['grade', 'tutor', 'createdAt', ];
  ...
  static getInstance() {
    ...
    return new EnrollmentListComponent(null, null, null, null, null);
  }
}