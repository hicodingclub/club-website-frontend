...
@Directive({
  ...
  providers: [{
    ...
    useExisting: TeachforlifeTutorDirectiveEmail,
    multi: true
  }]
})
      ...
      for (let idx = 0; idx < this.validators.length; idx++) {
        ...
        try {
          ...
          if (result == false) return {
            'teachforlifeTutorDirectiveEmail': obj.msg
          };
        } catch (e) {
          return {
            'teachforlifeTutorDirectiveEmail': obj.msg
...
@Directive({
  ...
  providers: [{
    provide: NG_VALIDATORS,
    useExisting: TeachforlifeTutorDirectivePhoneNumber,
    multi: true
  }]
})
    ...
    if (typeof value == 'string') {
      for (let idx = 0; idx < this.validators.length; idx++) {
        ...
        try {
          result = obj.validator(value)
          if (result == false) return {
            'teachforlifeTutorDirectivePhoneNumber': obj.msg
          };
        } catch (e) {
          return {
            'teachforlifeTutorDirectivePhoneNumber': obj.msg
